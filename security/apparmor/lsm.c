				...
				size_t *size)
{
	...
	if (label) {
		...
		if (error > 0)
			lsm_fill_user_ctx(lx, value, error, LSM_ID_APPARMOR, 0);
	}
...
static int apparmor_setselfattr(u64 __user attr, struct lsm_ctx __user *ctx)
{
	...
	if (lctx.ctx_len == 0 || lctx.ctx_len > lctx.len)
		return -EINVAL;

	context = kmalloc(lctx.ctx_len, GFP_KERNEL);
	if (context == NULL)
		return -ENOMEM;

	if (copy_from_user(context, ctx + 1, lctx.ctx_len)) {
		rc = -EFAULT;
		goto out;
	}

	rc = do_setattr(attr, context, lctx.ctx_len);
out:
	kfree(context);
	return rc;
}
