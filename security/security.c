...
void security_cred_getsecid(const struct cred *c, struct lsmblob *l)
{
	...
	hlist_for_each_entry(hp, &security_hook_heads.cred_getsecid, list)
		hp->hook.cred_getsecid(c, &l->secid[hp->secidat]);
}
...
int security_kernel_act_as(struct cred *new, struct lsmblob *l)
{
	...
	hlist_for_each_entry(hp, &security_hook_heads.kernel_act_as, list) {
		rc = hp->hook.kernel_act_as(new, l->secid[hp->secidat]);
		if (rc != 0)
			return rc;
	}
	return 0;
...
void security_task_getsecid(struct task_struct *p, struct lsmblob *l)
{
	...
	hlist_for_each_entry(hp, &security_hook_heads.task_getsecid, list)
		hp->hook.task_getsecid(p, &l->secid[hp->secidat]);
}
...
void security_ipc_getsecid(struct kern_ipc_perm *ipcp, struct lsmblob *l)
{
	...
	hlist_for_each_entry(hp, &security_hook_heads.ipc_getsecid, list)
		hp->hook.ipc_getsecid(ipcp, &l->secid[hp->secidat]);
}
			      ...
			      void *lsmrule)
{
	...
	hlist_for_each_entry(hp, &security_hook_heads.audit_rule_match, list) {
		rc = hp->hook.audit_rule_match(l->secid[hp->secidat], field,
					       op, lsmrule);
		if (rc != 0)
			return rc;
	}
