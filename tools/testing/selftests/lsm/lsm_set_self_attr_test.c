...
{
	ASSERT_EQ(-1, syscall(__NR_lsm_set_self_attr, NULL, LSM_ATTR_CURRENT));
...
TEST(size_too_small_lsm_set_self_attr)
{
	...
	ASSERT_GE(1, syscall(__NR_lsm_get_self_attr, ctx, &size,
			     LSM_ATTR_CURRENT));
	ctx->len = 1;
	ASSERT_EQ(-1, syscall(__NR_lsm_set_self_attr, ctx, LSM_ATTR_CURRENT));
...
TEST(flags_zero_lsm_set_self_attr)
{
	...
	ASSERT_GE(1, syscall(__NR_lsm_get_self_attr, ctx, &size,
			     LSM_ATTR_CURRENT));
	ASSERT_EQ(-1, syscall(__NR_lsm_set_self_attr, ctx, 0));
	ASSERT_EQ(EINVAL, errno);
...
TEST(flags_overset_lsm_set_self_attr)
{
	...
	ASSERT_GE(1, syscall(__NR_lsm_get_self_attr, tctx, &size,
			     LSM_ATTR_CURRENT));
	ASSERT_EQ(-1, syscall(__NR_lsm_set_self_attr, tctx,
	...
	if (cnt_current) {
		...
		for (i = 0; i < count; i++) {
			...
			case LSM_ID_SELINUX:
				...
				rc = syscall(__NR_lsm_set_self_attr, tctx,
					     LSM_ATTR_CURRENT);
				ASSERT_EQ(0, rc);
				tctx->ctx[0] = 'X';
				rc = syscall(__NR_lsm_set_self_attr, tctx,
